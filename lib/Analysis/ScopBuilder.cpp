...
bool isValueFortranArray(Value *V) {
  ...
  assert(Ty != nullptr && "Ty expected to be initialized");
...
Value *ScopBuilder::findFADGlobalNonAlloc(MemAccInst Inst) {
  ...
  for (auto user : MallocMem->users()) {
    ...
    if (!isValueFortranArray(Descriptor))
      continue;
...
/// passes have been run.
///
/// This is trying to match against %param, the descriptor
/// of the Fortran array that is being accessed at load/store. This style
/// of code is generated for arrays that are local, generated as parameters
/// to functions.
///
/// Pattern Match:
