...
; Verify that we emit correct scalar dependences in case the abstract matrix
; call is in a different basic block.

; CHECK:    Arrays {
; CHECK-NEXT:        float* MemRef_call; // Element size 8
...
; CHECK-NEXT:    	Stmt_for_tmp
; CHECK-NEXT:            Domain :=
; CHECK-NEXT:                { Stmt_for_tmp[i0, i1] : 0 <= i0 <= 1023 and 0 <= i1 <= 1023 };
; CHECK-NEXT:            Schedule :=
; CHECK-NEXT:                { Stmt_for_tmp[i0, i1] -> [i0, i1, 0] };
; CHECK-NEXT:            MustWriteAccess :=	[Reduction Type: NONE] [Scalar: 1]
; CHECK-NEXT:                { Stmt_for_tmp[i0, i1] -> MemRef_call[] };
; CHECK-NEXT:    	Stmt_for_body3
; CHECK-NEXT:            Domain :=
; CHECK-NEXT:                { Stmt_for_body3[i0, i1] : 0 <= i0 <= 1023 and 0 <= i1 <= 1023 };
; CHECK-NEXT:            Schedule :=
; CHECK-NEXT:                { Stmt_for_body3[i0, i1] -> [i0, i1, 1] };
; CHECK-NEXT:            ReadAccess :=	[Reduction Type: NONE] [Scalar: 0]
; CHECK-NEXT:                { Stmt_for_body3[i0, i1] -> MemRef_A[i0, i1] };
; CHECK-NEXT:            ReadAccess :=	[Reduction Type: NONE] [Scalar: 1]
; CHECK-NEXT:                { Stmt_for_body3[i0, i1] -> MemRef_call[] };
...
for.cond1:                                        ; preds = %for.inc, %for.body
  ...
  br i1 %exitcond, label %for.tmp, label %for.end

for.tmp:                                        ; preds = %for.cond1
