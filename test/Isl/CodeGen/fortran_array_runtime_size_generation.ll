; Check that the runtime size computation is generation for fortran arrays
; PPCG code generation backend:
; RUN: opt %loadPolly -S -polly-process-unprofitable -polly-allow-nonaffine \
; RUN:        -polly-ignore-aliasing -polly-invariant-load-hoisting \
; RUN:        -polly-canonicalize -polly-target=gpu -polly-acc-dump-kernel-ir \
; RUN:        -polly-acc-mincompute=0 -polly-detect-fortran-arrays -polly-codegen-ppcg \
; RUN:         < %s | FileCheck %s 

; Reguar code generation backend:
; RUN: opt %loadPolly -S -polly-process-unprofitable -polly-allow-nonaffine \
; RUN:        -polly-ignore-aliasing -polly-invariant-load-hoisting \
; RUN:        -polly-canonicalize -polly-detect-fortran-arrays -polly-codegen < %s | FileCheck %s
