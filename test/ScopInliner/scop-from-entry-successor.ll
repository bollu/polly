...
; `inline_site`, when the `to_be_inlined` has a `Scop` from the *successor* of
; the entry block to the exit node.
...
;     int B; // we do not allow dead code elimination on purpose to have an alloca.
;     for(int i = 0; i < N; i++)
;         A[i] *= 10;
; }
;
...
entry:
  %alloc_to_block_modelling = alloca i32
