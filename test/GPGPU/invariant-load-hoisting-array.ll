...
; SCOP-NEXT:         Execution Context: [tmp1, tmp4] -> {  :  }
; SCOP-NEXT:         ReadAccess :=	[Reduction Type: NONE] [Scalar: 0]
; SCOP-NEXT:             [tmp1, tmp4] -> { Stmt_for_body[i0] -> MemRef_control[0] };
; SCOP-NEXT:         Execution Context: [tmp1, tmp4] -> {  :  }
; SCOP-NEXT: }

; HOST-IR: call void @polly_launchKernel(i8* %89, i32 %103, i32 1, i32 32, i32 1, i32 1, i8* %polly_launch_0_params_i8ptr)
; HOST-IR: call void @polly_freeKernel(i8* %89)

; void f(int *begin, int *end, int *arr, int *control) {
...
define void @f(i32* %begin, i32* %end, i32* %arr, i32* %control) {
entry:
